...
SCHEMA_PATH = os.path.join(SCRIPT_DIR, "..", "schemas", "model_audit_trail.schema.json")
...
def main():
    ...
    if args and args[0] == "--mongo":
        ...
        c = client["credentials"]["model_audit_trail"]
        total = c.count_documents({})
        print(f"Validating {total} docs in credentials.model_audit_trail...")
        for doc in c.find({}):
            label = f"{doc.get('provider','?')}/{doc.get('model_id','?')}/{doc.get('event_type','?')}"
        ...
        c = client["credentials"]["model_audit_trail"]
        total = c.count_documents({})
        print(f"Validating ALL {total} docs in credentials.model_audit_trail...")
        for doc in c.find({}):
            label = f"{doc.get('provider','?')}/{doc.get('model_id','?')}/{doc.get('event_type','?')}"
            do_validate(label, doc)
        print(f"Result: {errors_found}/{docs_checked} invalid")
        ...
        if not path or not os.path.exists(path):
            print(f"Usage: {sys.argv[0]} [--mongo|--all|<json_file>]")
            sys.exit(0)
        with open(path) as f:
            data = json.load(f)
        if isinstance(data, list):
            for doc in data:
                do_validate(doc.get("evidence_id", "?"), doc)
        elif isinstance(data, dict):
            do_validate(data.get("evidence_id", "file"), data)
