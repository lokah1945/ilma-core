#!/usr/bin/env python3
"""
model_benchmarks validator — jsonschema draft-07 with BSON normalization.
"""
import json, sys, os
...
SCHEMA_PATH = os.path.join(SCRIPT_DIR, "..", "schemas", "model_benchmarks.schema.json")
...
def main():
    ...
    if args and args[0] == "--mongo":
        ...
        c = client["credentials"]["model_benchmark"]  # FIX 2026-06-17: singular
        total = c.count_documents({})
        print(f"Validating {total} docs in credentials.model_benchmark...")
        for doc in c.find({}):
            label = f"{doc.get('provider','?')}/{doc.get('model_id','?')}/{doc.get('benchmark_source','?')}"
        ...
        if not docs:
            print("model_benchmark is EMPTY")
            client.close()
            sys.exit(0)
        ...
        c = client["credentials"]["model_benchmark"]  # FIX 2026-06-17: singular
        total = c.count_documents({})
        print(f"Validating ALL {total} docs in credentials.model_benchmark...")
        for doc in c.find({}):
            label = f"{doc.get('provider','?')}/{doc.get('model_id','?')}/{doc.get('benchmark_source','?')}"
            do_validate(label, doc)
        print(f"Result: {errors_found}/{docs_checked} invalid")
