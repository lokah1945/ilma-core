...
7. **systemd PYTHONPATH for profile-root imports** — If the backend service `WorkingDirectory` is the narrow `.../dashboard/backend` subdir (which it is), `from ilma_unified_cache import ...` (and any other profile-root module) fails at service runtime even though it imports fine in an interactive shell. The `/api/health` endpoint then reports `"unified_cache":false` and overall `"status":"degraded"`. Fix: add `Environment=PYTHONPATH=/root/.hermes/profiles/ilma` to the systemd unit, `systemctl --user daemon-reload`, restart, then assert `curl /api/health` → `"status":"ok"`. This is the dual of the stale-`.env` MongoDB-auth pitfall (see `ilma-system-audit` references) — both surface only via the app health endpoint, never via `systemctl is-active`.
...
7. **Vite binds to `[::1]` (IPv6 localhost) only by default — `127.0.0.1:3001` REFUSED.** This is the real reason "dashboard tidak muncul" when accessed via `127.0.0.1` or a LAN IP. The frontend systemd unit also hardcodes `--port 3001` while `vite.config.ts` says `port: 3000` + `strictPort` → port conflict forces 3001. **Fix (2026-07-28):** set `server.host: '0.0.0.0'` and `strictPort: true` in `vite.config.ts`, and change the unit `ExecStart` to `--port 3000 --host 0.0.0.0` so frontend runs on 3000 and is reachable from any interface. After change: `systemctl --user daemon-reload && systemctl --user restart ilma-dashboard-frontend.service`.
8. **"Site can't be reached" = localhost-bound services (not a crash).** If the dashboard or any ILMA service is unreachable from a LAN IP but `curl 127.0.0.1:PORT` works, the service is bound to `127.0.0.1`/`[::1]` only. Fix: change the systemd `ExecStart` `--host 127.0.0.1` → `--host 0.0.0.0` (uvicorn) and add `host:'0.0.0.0'` (Vite), `daemon-reload`, restart. Verify with `curl http://<LAN-IP>:PORT` → 200. All wrapper + dashboard units were switched to `0.0.0.0` on 2026-07-28 for this reason.
9. **Empty dashboard / 500 on `/api/*` = corrupt or empty `ilma_dashboard.db`.** Symptom: pages show "no record", or backend returns `HTTP 500` with `sqlite3.OperationalError: no such column: benchmark_records.canonical_model_id`. Root cause: stale DB after a backend schema change, OR empty tables because the seeder's source files moved. **Fix:** backup + delete `data/ilma_dashboard.db`, then run `python3 scripts/seed_dashboard_db.py` from `dashboard/backend/`. See `references/dashboard-db-recovery.md` for the exact recipe + the seeder source-path change (`model_specialization_database.json` no longer exists → seeder now falls back to `ilma_model_router_data/PROVIDER_INTELLIGENCE_MASTER.json`).
10. **CORS must allow the access origin.** If the frontend loads but every API call fails (CORS block), backend `allow_origins` is too narrow. The dashboard backend hardcodes `allow_origins=["http://localhost:3000","http://127.0.0.1:3000"]` — accessed from a LAN IP (e.g. `http://172.16.102.11:3000`) the browser is blocked. Fix: set `allow_origins=["*"]` (or add the LAN origin) in `dashboard/backend/app/main.py`, restart backend. (Security note: `*` is fine on a trusted LAN; add nginx + auth if exposed to the internet.)
...
## References
- `references/phase56-production-entrypoint.md`
- `references/dashboard-zero-error-audit.md`
- `references/dashboard-db-recovery.md` — empty/corrupt `ilma_dashboard.db` recovery: delete + reseed (seeder now reads `PROVIDER_INTELLIGENCE_MASTER.json`), plus bind/CORS fixes for LAN access.
