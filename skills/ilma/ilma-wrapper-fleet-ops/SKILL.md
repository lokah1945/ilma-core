...
| openrouter | 9106 | openrouter | yes (NEW from pull 5373697, 2026-07-28) |
...
0. **(If Bos says "backup dulu")** `tar czf` both `/root/project/mode_fetcher` and `/root/wrapper` to `/root/backup_*.tar.gz` BEFORE pulling. See references § Backup-before-massive-pull.
...
11. **OpenRouter wrapper startup fails (NEW from pull `5373697`, 2026-07-28)** — 4-layer failure, see `references/nvidia-nim-fetcher-ops.md` § "OpenRouter wrapper". Summary: (a) shipped systemd unit has `ProtectHome=true`+`ProtectSystem=strict` → blocks `/root/wrapper`; delete them. (b) `PYTHONPATH=/root/wrapper/openrouter` → must be `/root/wrapper` (so `openrouter/` + `common/` resolve). (c) `WantedBy=multi-user.target` → use `default.target` (user systemd has no multi-user). (d) **UPSTREAM BUG** `openrouter/src/main.py:491` `RequestSizeLimiter(max_size=...)` → `max_bytes=` (signature in `common/middleware.py`). This is a legit upstream fix — commit + push to github (done: `f95921a`). After fixes: `systemctl --user enable --now openrouter.service` → 9106 /health 200.
   **ALSO pre-auth**: openrouter `.env` ships `BEARER_TOKEN=your-secure-random-token-here`. Apply the SAME pre-auth treatment as the 5 others: set `BEARER_TOKEN=wrapper-local-key` + `DISABLE_AUTH=1` (openrouter already honors `DISABLE_AUTH` at ~line 496/505). Without this, `/v1/models` returns 401. After: `curl http://127.0.0.1:9106/v1/models` → lists models with NO token.
12. **Cloudflare 1010 bot-block on default Python/aiohttp UA (opencode 9103 + vercel 9105, 2026-07-28, commit 5bb2e92)** — wrapper fails EVERY request with `503 No capacity` / `invalid_credential` / `Not Found`, but `curl` to the same upstream with the same key = 200. Root cause: Cloudflare anti-bot block on the default `Python/3.x aiohttp/x.x.x` User-Agent (`HTTP 403 error code: 1010`). The wrapper marks the key invalid → misleading failure. **Fix**: add a browser `User-Agent` (`Mozilla/5.0 ... Chrome/124.0.0.0 Safari/537.36`) + `Accept-Encoding: gzip, deflate` (DROP `br`) to the `aiohttp.ClientSession(headers=...)` in `<wrapper>/src/main.py` (the session-init block, ~line 382 for opencode, ~line 382 for vercel), restart. After: opencode `hallo` → "Hallo! Wie..." (verified). Full repro + diff in `ilma-wrapper-troubleshooting` → `references/cloudflare-1010-ua-block.md`. KEY RULE: if `curl` works but aiohttp/urllib fails with 403/1010 or fake `invalid_credential`, SUSPECT UA-block BEFORE rotating keys. (Vercel's remaining 403 after the UA fix is a SEPARATE upstream cause — AI Gateway requires a credit card on file — NOT a UA issue.)
...
- After ANY wrapper change, verify ALL 7 ports (9101–9105 + 9106 openrouter) return 200, not just the one you touched — a restructure commit can break siblings. Also hit `/v1/models` with an `x-request-id` header to exercise the latency middleware (catches the `[wrapper]` NameError that `/health` alone may miss).
...
- **Backup BEFORE a "massive" pull.** When Bos says "backup dulu kedua project karena update massive", `tar czf` BOTH `/root/project/mode_fetcher` and `/root/wrapper` to `/root/backup_*.tar.gz` (with `--warning=no-file-changed` for wrapper's live `model-state.db`) BEFORE `git pull`. See `references/nvidia-nim-fetcher-ops.md` § "Backup-before-massive-pull".
- **NEVER `git push` a `.env` containing API keys.** The fetcher repo tracks `.env` (upstream). Pushing it leaks Bos's keys to GitHub. After key-injection into `mode_fetcher/.env`, leave it UNCOMMITTED. SOT (MongoDB) auth FAILED this session (stale token) — fallback to scraping wrapper `.env` files. See `references/nvidia-nim-fetcher-ops.md` § "API Key Injection".
- **`mode_fetcher` is the renamed `nvidia-nim_model_fetcher`** (Bos 2026-07-28). Pull from `origin` (NOT `github`). MCP server renders `/dashboard` LIVE from DB — restart the orphan process after pull (no systemd unit). See `references/nvidia-nim-fetcher-ops.md`.
- **OpenRouter (9106) is a NEW wrapper from pull `5373697`.** Not pre-installed: needs venv + systemd unit + 4 bug-fixes (see bug #11). After setup: `systemctl --user enable --now openrouter.service`.
- **Wrapper-bug vs upstream-account-limit.** A 4xx/5xx after restart is NOT automatically a wrapper bug. `503 No capacity` / `503 circuit breaker open` / `403 requires credit card` = upstream account/quota limit (opencode, vercel this session). Verify by hitting the upstream directly with the same key. Don't "fix" the wrapper for upstream limits — see `references/upstream-account-limit-vs-wrapper-bug.md`.
...
- `references/nvidia-nim-fetcher-ops.md` — SEPARATE repo `/root/project/mode_fetcher` (RENAMED from nvidia-nim_model_fetcher; pull from `origin`, NOT `github`); MCP server on 9100 renders dashboard LIVE from DB; manual restart procedure (no systemd unit); **API key injection from wrapper `.env`**; **OpenRouter 9106 new-wrapper setup (4 bug-fixes)**; **backup-before-massive-pull recipe**.
- `references/upstream-account-limit-vs-wrapper-bug.md` — decision tree to tell wrapper bugs (port/import/health/401) from upstream account limits (503 No capacity, 403 needs CC); 2026-07-28 all-wrapper "hallo" test matrix + MCP fetcher JSON-RPC test recipe.
...
- `references/port-conflict-restart.md` — port conflict during restart (address already in use), diagnosis with `lsof`/`ss`, resolution steps, prevention pattern template for stale process cleanup.
- `scripts/verify_wrappers.py` — probe all wrapper ports, print health table.
- `scripts/smoke_hallo_all.py` — **end-to-end chat smoke**: sends "hallo" to all 5 wrappers, picks first `:free` model, reports non-empty reply or error class (401/0-models/exhausted/No-capacity). Use this (not manual curl loops) to verify chat works after a pull/config change.
