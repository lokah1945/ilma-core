...
### P-114: wrapper-nvidia consumer availability — 6-point audit

A wrapper being live (`/v1/models` returns 132 models, inference works)
does NOT mean all consumers can use it. Each consumer has its own
credential lookup, endpoint map, and model-ID strip logic. When Bos
asks "pastikan wrapper-nvidia tersedia untuk semua consumer", run the
**6-point consumer audit**:

| # | Consumer | Check | Common failure |
|---|----------|-------|----------------|
| 1 | `ilma_provider_kernel.py` | Is there an `elif provider == "wrapper-nvidia"` branch? | Only `nvidia` branch → routes to remote `integrate.api.nvidia.com` |
| 2 | `ilma_credentials_v2.py` | Does `get_credential("wrapper-nvidia")` return a key? | No handler → returns None → "No API key configured" |
| 3 | `ilma_claudecode_agent.py` | After `provider = model_id.split("/")[0]`, is `nvidia` remapped to `wrapper-nvidia`? | No remap → uses remote NVIDIA key |
| 4 | `ilma_claudecode_agent.py` strip prefix | Does `model_id.startswith(provider + "/")` work AFTER remap? | `provider` changed to `wrapper-nvidia` but `model_id` still starts with `nvidia/` → strip fails → sends `nvidia/meta/...` to wrapper → 404 |
| 5 | Dashboard DB `model_ids` table | Are wrapper-nvidia models synced? | SPEC_DB file missing → ingestion skips → 0 models → kanban optimizer gets 0 |
| 6 | `PROVIDER_INTELLIGENCE_MASTER.json` | Are models `disabled: False`? | All 132 models `disabled: True` despite being live → `is_model_runtime_allowed` returns False → every consumer blocked |

**Fix sequence** (all verified 2026-07-18, commit `d1c946e`):
1. `ilma_provider_kernel.py`: add `elif provider == "wrapper-nvidia"` → `http://127.0.0.1:9100/v1/chat/completions`, `api_key or "wrapper-local-key"`.
2. `ilma_credentials_v2.py`: add `if provider.lower() == "wrapper-nvidia": return "wrapper-local-key"` (local non-secret key).
3. `ilma_claudecode_agent.py`: add `if provider == "nvidia": provider = "wrapper-nvidia"` before the dispatch check.
4. `ilma_claudecode_agent.py`: fix strip — use `orig_prefix = model_id.split("/")[0] + "/"` (from ORIGINAL model_id, before remap), then `api_model = model_id[len(orig_prefix):]`.
5. Run `scripts/ilma_wrapper_nvidia_dashboard_sync.py` to populate `model_ids` table from live `/v1/models`.
6. `PROVIDER_INTELLIGENCE_MASTER.json`: set `disabled=False, is_active=True` for all 132 wrapper-nvidia models (backup first).

**E2E verification** (7 consumers, all must pass):
```python
# provider_kernel, subagent_router, claudecode_agent, kanban, dashboard_db,
# router_runtime_allowed, sot_credential
```
See `references/wrapper-nvidia-consumer-audit-2026-07-18.md` for the full
fix sequence with code snippets and the E2E test script.

### P-115: PROVIDER_INTELLIGENCE_MASTER.json disabled flag drift

The master JSON can have `disabled: True` on models that are actually
live and inference-capable. This happens when a sync/enrichment step
marks models disabled (e.g. provider-level disable propagated to all
models) but a later step re-enables the provider without clearing the
per-model flag. **Always verify `disabled` per-model, not per-provider.**
Symptom: `is_model_runtime_allowed()` returns False (line 516 checks
`found.get("disabled", False)`) even though the model responds to
inference calls. Fix: iterate all models under the provider and set
`disabled=False` where the live endpoint confirms availability.
