...
### Playwright → Puppeteer-Extra Migration

Boss confirmed: use `puppeteer-extra-plugin-stealth` (Node.js) NOT `playwright_stealth` (Python). ILMA's script `ilma_codex_oauth.js` is the Node.js implementation that replaced `ilma_codex_oauth.py`.

### Browser Detection Args TO REMOVE

Boss confirmed these args trigger OpenAI detection:
- `--no-sandbox`
- `--disable-blink-features=AutomationControlled`

They should be removed from any browser launch configuration for OAuth.
...
This reaches the email entry page but typing via CDP produces empty input.
...
1. `/root/.openclaw/agents/main/agent/auth-profiles.json` — primary (OpenClaw format)
2. `/root/.hermes/profiles/ilma/scripts/.codex_tokens.json` — local backup

---

## Next Steps

1. **Accept copy-paste flow** as primary method — document clearly for Boss
...
- `/root/.hermes/profiles/ilma/scripts/ilma_codex_oauth.js` — Node.js OAuth script (v3, v4 iterations)
- `/root/.hermes/profiles/ilma/skills/ilma/ilma-codex-integration/SKILL.md` — Updated with root cause findings
- `/tmp/codex_google_flow.js` — Debugging script (discarded)
...
- **Root cause identified:** YES — OpenAI server-side detection
- **Working alternative found:** YES — copy-paste URL
- **Bottleneck resolved:** NO — requires Boss decision on next steps (provide password, export cookies, or accept copy-paste)