...
triggers:
  ...
  - "fix bug"
  - "fix semua bug"
  - "tidak ada toleransi bug"
  - "audit end-to-end sekarang"
version: 1.4.0
...
### P-20: "Fix bug" tanpa spec jelas = AUDIT-FIRST, JANGAN patch non-bugs

**Trigger**: Bos bilang "fix bug", "tidak ada toleransi bug apapun", "audit end-to-end sekarang", atau bentuk tekanan apapun untuk menulis kode memperbaiki bugs yang belum terbukti ada di code core.

**Perangkap nyata** (konkret, Sesi 2026-07-07 "fix bug"):

1. Bos: *"fix bug, tidak ada toleransi bug apapun"*
2. Saya grep `agent.log` dan lihat N baris error dari 7 hari terakhir
3. Saya LANGSUNG asumsikan ini bug di code `ilma_*` core
4. Saya mulai draft patch
5. **Tapi saya TIDAK verify**: bug ini beneran ada di code core, ATAU di scratch heredoc yang saya tulis dan lupa, ATAU di transient error masa lalu dari upstream yang tidak lagi reproducible?

**Apa yang verification actually proves** (dari sesi yang sama):

| Step | Tool | Temuan | Verdict |
|---|---|---|---|
| Re-grep `Traceback` dari sesi 07-07 | `grep` | 5+ matches, semuanya dari `tool_executor returned error` di inline `python3 << EOF` heredocs | BUKAN bug agent core — script itu lenyap begitu heredoc berakhir |
| Re-grep `Traceback` dari 07-01/02/04 | `grep` | Semua `ImportError` dari `ilma_mongo_config.get_db`/`get_mongo_client` — function itu sekarang tidak ada karena module di-refactor ke `get_local_client`/`get_remote_client` | BUKAN bug — nama refactor-out; error transient, module kini sehat |
| `import ilma_provider` di shell | `python3 -c 'importlib.import_module("ilma_provider")'` | `ModuleNotFoundError: No module named 'ilma_provider'` | Terdengar parah, TAPI grep semua `.py` untuk `import ilma_provider` return ZERO caller-chain — semua reference koment atau `ilma_provider_kernel` (yang memang ADA). **False positive dari grep over-broad.** |
| `from ilma_workflow_ecc import route_task` | `python3 -c '...'` | `ImportError: cannot import name 'route_task'` | Function ada di `ilma_model_router.route_task`, BUKAN `ilma_workflow_ecc` — saya salah pilih source module. Code intact, test saya yang salah. |
| `from ilma_judge_system import JudgeSystem` | `python3 -c '...'` | `ImportError` | Class di-wrap di belakang `get_judge_system()` yang return `JudgeSystemWrapper` — public API-nya function, bukan class. Test saya import simbol yang salah. |

Setelah SEMUA verification, state code agent core:
- 21/21 critical modules import OK ✅
- `ilma.py --status` return READY ✅
- `ILMAOrchestrator()` instantiate dengan attribute `execution_log` ✅
- `CapabilityRegistry.list_all()` return 37 capabilities ✅
- `SubAgentRouter.route('hello')` return model yang jalan ✅

**Kesimpulan**: **TIDAK ada bug core-agent yang butuh patch.** Semua "bug" yang saya flag di mental pre-flight adalah false positive dari pilihan-import yang buruk di script audit saya sendiri.

**Rule (Lock)**:

1. **"Fix bug" adalah command state-verify-first, BUKAN write-code-first.** Selalu verify bug exist di runtime SEBELUM mikirin patch.
2. **Verify dengan FRESH import + signature grep**, BUKAN dari memory. Memory bilang "import X works" — runtime bilang "ImportError".
3. **Saat verification nunjukin false positive, JANGAN tulis fix.** Melaporkan false positive ITU SENDIRI adalah fix (itu audit report yang Bos minta).
4. **Anti-pattern**: Di system prompt saya pernah menulis akan *selalu jalankan patch* di setiap audit finding. Patch-from-audit itu SALAH kalau audit-nya sendiri salah. Audit dulu, fix hanya yang survive audit.
5. **Layer audit berbeda tangkap hal berbeda** — pakai urutan ini:
   ```bash
   # Layer 1: import test (tangkap module non-existent)
   python3 -c 'importlib.import_module("ilma_<thing>")'

   # Layer 2: SIAPA yang actually import (tangkap caller yang terlihat valid)
   grep -rn "from ilma_<thing> import\|import ilma_<thing>" --include="*.py" .

   # Layer 3: signature check (tangkap wrong-symbol import)
   grep -nE "^def [a-z_]+|^class [A-Z]" ilma_<thing>.py | head -10

   # Layer 4: real API smoke (tangkap runtime wrong-arg-type)
   python3 -c "from ilma_<thing> import <known_public_api>; <known_public_api>(...)"
   ```

6. **Kalau verification pass nunjukin bug tidak recur di runtime**, **berhenti lapor.** Bos's "no tolerance to bugs" dipuaskan oleh REPORT YANG IDENTIFY ZERO BUG secara benar, BUKAN oleh patch yang diarang-arangan yang nyentuh code sehat dan bikin regression.

7. **Hedge language pattern** (Bos appreciate kalau saya pakai ini, 2026-07-07):
   > "Saya audit real-API, berikut kategorinya: (a) upstream NIM (DEGRADED + 429), bukan core bug ILMA; (b) scratch heredoc dari sesi sebelumnya yang tidak exist lagi; (c) 0 critical di core agent. Tidak ada patch yang dibutuhkan untuk core code."

**Related**: Pitfall ini adalah runtime-version dari P-2 (Memory ≠ ground truth) — diterapkan ke *bug reports*, bukan cuma *config values*. P-2 cover "model/provider apa yang dipakai?"; P-20 cover "bug apa yang ada di code?". Same root cause (assume vs verify), different domain.

**Lihat juga**: `references/audit-false-positive-pattern-2026-07-07.md` — transcript konkret dari failed imports + recipe yang buktikan mereka false positives.

### P-21: READ-ONLY AUDIT + HANDOFF SOP (wrapper/agent ecosystem audit)

**Trigger**: Bos minta "audit mendalam, read-only, jangan sentuh code, eksekusi oleh agent lain", atau "siapkan tugas di /root/audit_report, agent lain yang jalankan".

**SOP (Lock)**:
1. **ILMA = auditor + planner, BUKAN executor.** Saat Bos bilang "dilarang menyentuh code / read only", `patch`/`write_file` ke target project (mis. `/root/wrapper/*`) = **DILARANG**. Hanya `terminal` (curl/read), `read_file`, `search_files`, dan tulis ke `/root/audit_report/`.
2. **Report HARUS di `/root/audit_report/`** — BUKAN di dalam project (`/root/wrapper/audit_report/` salah, agent sebelumnya pernah salah naruh). Path absolut dari `/root`.
3. **Versioning Vn**: audit iterasi dinamai V1→Vn berurutan. Jangan salah nama (Bos tegur "kok v2? harusnya v3"). Hitung: V1=initial, V2=post-patch re-audit, V3=final verify, dst.
4. **Cleanup sebelum Vn baru**: hapus report obsolete (V1-Vn-1 planning) dari `/root/audit_report/`, KEEP hasil eksekusi agent (`V{n-1}_EXECUTION_REPORT.md`) sebagai referensi, lalu tulis `Vn_AUDIT.md` + `Vn_PLAN.md`.
5. **Handoff**: `Vn_PLAN.md` berisi instruksi eksplisit (P0/P1/P2 + file + verify command) untuk agent lain execute. Jangan execute sendiri.

**Anti-pattern**: nulis report ke `/root/wrapper/audit_report/`; edit source project saat mode read-only; skip cleanup lalu numpuk file.

### P-22: SDK-COMPAT AUDIT TECHNIQUE (OpenAI/Anthropic-compatible wrapper)

**Trigger**: audit wrapper yang klaim "OpenAI/Anthropic SDK compatible" / "production ready 100/100".

**Curl matrix wajib** (per wrapper, port 910x). Reference: `references/sdk-compat-audit-matrix.md` (script template + findings pattern).

| Kategori | Test | Expected | Celah umum |
|----------|------|----------|-----------|
| Chat | explicit + alias `sonnet` | 200 | alias resolve gagal → 404/400 |
| Chat | empty messages / malformed JSON | 400 | 500 = bug |
| Chat | `tools` + `tool_choice:auto` | 200 | crash 500 |
| Chat | `stream:true` | `data:` lines + `[DONE]` | 0 lines = broken |
| Responses | `model` + `input` | 200, `object:response` | format beda |
| Anthropic | `messages` + `tools` | 200 | `isinstance()` typo → 500 |
| Anthropic | `system` as array, `thinking` | 200 | |
| Anthropic | `x-api-key` auth | 200 | hanya `Authorization` didukung |
| Anthropic | missing `anthropic-version` | 400 | 404 = bug |
| CORS | `OPTIONS` preflight | 200 + `access-control-allow-origin` | 401/400 = browser SDK gagal |
| Auth | no token | 401 `authentication_error` | |
| Error | bad model | 400 `invalid_request_error` | upstream `404 page not found` = bug |
| Usage | non-stream chat | `usage` field present | MISSING = OpenAI-violation |
| Concurrency | ×3 parallel | 200×3 | race/leak |

**Gotcha terbukti (V1→V5):**
- **systemd `EnvironmentFile=` missing** → `.env` var (`DYNAMIC_ALIAS_TARGET`) tidak kebaca saat restart → alias `sonnet`=None → 404. Cek: `grep -c EnvironmentFile /root/.config/systemd/user/<unit>.service`.
- **Alias resolver cache invalid model** → health `dynamic_alias_target` jadi `nonexistent-model-xyz` (pollution). Fix: reject unknown, jangan cache.
- **CORS preflight** butuh `allow_origin_regex` localhost + auth middleware skip `OPTIONS`.
- **`isinstance(tools)` typo** → `TypeError` 500 (harus `isinstance(tools, list)`).

### P-23: POST-AUDIT REPORT OVERCLAIM — RE-VERIFY INDEPENDENTLY

**Trigger**: ada `Vn_EXECUTION_REPORT.md` dari agent lain yang klaim "score X/100, all fixed".

**Rule (Lock)**: JANGAN percaya klaim. Selalu re-verify via curl E2E sebelum lanjut.
- V4 audit: post-audit claim **93/100** → independent verify **86** (alias cold-start gap).
- V5 audit: post-audit claim **~97/100** → independent verify **nvidia 100 tapi nous/opencode ~75** (RC-2 NOT applied ke nous/opencode + `usage` MISSING).
- False-negative sendiri: tes terlalu cepat pasca-restart (uptime 2s) → 404/000. Tunggu warmup 8s sebelum claim.

**Pattern**: baca report → jalankan curl matrix mandiri → bandingkan → catat discrepancy di `Vn_AUDIT.md` → baru plan Vn+1.

**Anti-pattern**: "agent bilang 100, berarti beres" → lanjut tanpa verify → bug lolos ke production.
...
- `references/audit-false-positive-pattern-2026-07-07.md` — konkret transcript sesi 2026-07-07 di mana saya reflex "fix bug" menemukan 5 false positives (salah import, scratch heredoc lenyap, nama function yang sudah di-refactor-out). Recipe 4-layer verify + hedge language pattern.
- `references/sdk-compat-audit-matrix.md` — curl E2E matrix template untuk audit wrapper OpenAI/Anthropic-compatible + pattern findings (V1→V5). Load sebelum audit production-readiness wrapper.
