...
## Codex CLI e2e with custom wrapper provider (2026-07-23)
`codex exec` (v0.145.0) can target a local wrapper via `CODEX_HOME` isolation + base URL.

```bash
# isolated config
mkdir -p /root/.codex-homes/<wrapper>
cat > /root/.codex-homes/<wrapper>/config.toml <<'EOF'
model = "opencode/deepseek-v4-flash-free"
model_provider = "<wrapper>"
model_catalog_json = "/root/.codex-homes/<wrapper>/model_catalog.json"
EOF

# run against local wrapper on :9107
rm -f /tmp/codex_<wrapper>.txt
CODEX_HOME=/root/.codex-homes/<wrapper> timeout 120 \
  codex exec --model opencode/deepseek-v4-flash-free \
  "Use the Bash tool to run exactly: echo OK > /tmp/codex_<wrapper>.txt" </dev/null
```

**Pitfalls:**
- ❌ Missing `</dev/null` → Codex hangs on "Reading additional input from stdin..."
- ❌ `model_catalog.json` missing → `No such file or directory`
- ❌ catalog missing fields `shell_type`, `base_instructions` → JSON parse error. Clone a full entry from an existing catalog (`model_catalog.json.bak`) and change only `slug`/`display_name`/`description`/`known_aliases`.
- ❌ Model not in catalog → `Model metadata not found` → Codex treats it as non-tool-capable → never calls Bash.
- ⚠️ Free-tier Zen models may 400/429 on tool-use (upstream limitation, not wrapper bug). Verify wrapper with direct `curl /v1/responses` text first.
