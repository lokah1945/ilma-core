...
class SubAgentRouter:
    ...
    def route_and_execute(
        ...
        MAX_EXECUTION_TIME = 120  # 2 minutes maximum wall-clock

        tried: set = set()  # Models that failed (tracked persistently)

        while len(tried) < 8:  # Safety limit — cap attempts to bound wall-clock
    ...
    def _execute(self, model: str, message: str, thinking: str, stateless: bool, provider: str = "") -> Dict[str, Any]:
        ...
        if provider in _DIRECT_ENDPOINTS_DP:
            result = self._call_direct(provider, model, message, timeout=25)
        else:
            result = {
                "success": False,
                "content": "",
