...
def _resolve_remote_from_sot():
    ...
    try:
        ...
        # ROBUSTNESS PATCH (2026-07-24): local mongod may be no-auth.
        # Probe no-auth first, fall back to credentialed only if needed.
        c = None
        try:
            ...
            try:
                ...
                import sys as _sys
                print(f"[SOT] local probe failed: {_ce}", file=_sys.stderr)
                return None
        ...
        # FIX 2026-07-09 (audit F4): do not silently swallow SOT lookup errors.
        # Log so a real failure (auth/network/schema) is visible instead of masking to .env fallback.
        import logging
        logging.getLogger("ilma.sync").warning(f"[SOT] remote cred lookup failed, falling back to .env: {_e}")
    return None



# FIX 2026-07-09 (audit F10): remote replica sets (rs0) enforce a server-side $jsonSchema
# validator on credential collections. SOT v3 introduced key_status values
# (e.g. MULTI_ACCOUNT_DEFAULT_VALID) absent from the legacy enum, causing reconcile
# writes to fail with WriteError 121. Sanitize docs before upsert to remote.
_VALID_KEY_STATUS = {"QUOTA_EXCEEDED", "INVALID", "SERVER_ERROR", "TIMEOUT", "VALID", "UNVERIFIED"}
def _sanitize_for_remote_validator(doc: dict) -> dict:
    """Drop/normalize fields that violate the remote collection $jsonSchema validator."""
    if not isinstance(doc, dict):
        return doc
    ks = doc.get("key_status")
    if ks is not None and ks not in _VALID_KEY_STATUS:
        # Enum violation → remap to closest valid state instead of failing the whole write
        doc = dict(doc)
...
def _safe_bulk_write(coll, batch, **kw):
    """bulk_write tolerant of remote validator enum violations on key_status."""
    try:
        return coll.bulk_write(batch, **kw)
    except __import__("pymongo").errors.WriteError as _we:
        if "Document failed validation" in str(_we) and "key_status" in str(_we):
            log.warning(f"[sync] bulk_write validator rejected, retrying with key_status sanitized: {_we}")
            sanitized=[]
            for op in batch:
                try:
                    op._doc = _sanitize_for_remote_validator(getattr(op,'_doc',{}))
                except Exception:
                    pass
                sanitized.append(op)
            return coll.bulk_write(sanitized, **kw)
        raise

def _safe_update_one(coll, filter_, set_obj, **kw):
    """update_one tolerant of remote validator enum violations on key_status."""
    try:
        return coll.update_one(filter_, set_obj, **kw)
    except __import__("pymongo").errors.WriteError as _we:
        if "Document failed validation" in str(_we) and "key_status" in str(_we):
            # strip the offending key_status set and retry
            bad = set_obj.get("$set", {})
            if "key_status" in bad:
                log.warning(f"[sync] validator rejected key_status update, dropping field: {_we}")
                clean = {k:v for k,v in bad.items() if k!="key_status"}
                set_obj = {"$set": clean}
                if "$unset" in set_obj:
                    set_obj["$unset"]["key_status"]=""
                else:
                    set_obj["$unset"]={"key_status":""}
                return coll.update_one(filter_, set_obj, **kw)
        raise

def _safe_replace_one(coll, filter_, doc, **kw):
    """replace_one that tolerates remote validator errors by sanitizing and retrying once."""
    try:
        return coll.replace_one(filter_, doc, **kw)
    except __import__("pymongo").errors.WriteError as _we:
        if "Document failed validation" in str(_we):
            sanitized = _sanitize_for_remote_validator(doc)
            log.warning(f"[sync] validator rejected doc, sanitized key_status and retried: {_we}")
            return coll.replace_one(filter_, sanitized, **kw)
        raise
...
def _get_local_client() -> MongoClient:
    ...
    ROBUSTNESS PATCH (2026-07-24): local mongod may run WITHOUT auth
    (no-auth bind). Try no-auth first; fall back to credentialed connect
    only if the no-auth probe fails. This prevents the daemon from dying
    on 'Authentication failed' when .env local password is stale/mismatched
    but the instance is actually open.
    """
    # 1) Try no-auth (local is often open)
    try:
        _c = MongoClient(
            host=LOCAL_HOST,
            port=LOCAL_PORT,
        ...
        pass
    # 2) Fall back to credentialed connect
    return MongoClient(
        host=LOCAL_HOST,
        port=LOCAL_PORT,
        username=LOCAL_USER,
        ...
        if operation_type == "insert":
            ...
            try:
                ...
                if _get_conflict_policy(db_name) == "local_wins" and direction == "local_to_remote":
                    ...
                    _safe_replace_one(target_coll,{"_id": doc_id_val}, doc, upsert=True)
                    return coll_name
            ...
            result = _safe_update_one(target_coll, {"_id": doc_id}, set_obj, upsert=False)
            if result.matched_count == 0 and full_doc:
                # Doc doesn't exist on target — upsert full doc
                doc = copy.deepcopy(full_doc)
                doc.pop("_id", None)
                doc = _add_sync_metadata(doc, source_tag)
                _safe_replace_one(target_coll,{"_id": doc_id}, doc, upsert=True)
            return coll_name
        ...
        for coll_name in sorted(collections):
            ...
            for doc in remote_coll.find().batch_size(BULK_BATCH_SIZE):
                ...
                if len(batch) >= BULK_BATCH_SIZE:
                    try:
                        _safe_bulk_write(local_coll, batch, ordered=False)
                ...
                for doc_id in common:
                    ...
                    if local_h != remote_h:
                        ...
                        if policy == "local_wins":
                            ...
                            _safe_replace_one(remote_coll, {"_id": doc_id}, doc, upsert=True)
                        else:
                            # Push remote → local
                ...
                if only_local:
                    if policy == "local_wins":
                        ...
                        for doc in local_coll.find({"_id": {"$in": list(only_local)}}).batch_size(BULK_BATCH_SIZE):
                            ...
                            if len(batch) >= BULK_BATCH_SIZE:
                                try:
                                    _safe_bulk_write(remote_coll, batch, ordered=False)
                                    db_stats["fix_count"] += len(batch)
                                except Exception as e:
