...
def _process_change_event(
    ...
    # A document written by the sync engine will have _sync_source
    # set to the OPPOSITE side (e.g. local_to_remote sets "remote" 
    # on the target). So:
    #   - local_to_remote watcher: skip if _sync_source == "local_to_remote" (our own write) 
    #   - remote_to_local watcher: skip if _sync_source == "remote_to_local" (our own write)
    # User-written docs won't have _sync_source at all → always processed.
    full_doc = change.get("fullDocument") or {}
    event_source = full_doc.get(SYNC_SRC_FIELD, "")

    # Skip if this event was generated by the sync engine running in same direction
    if direction == "local_to_remote" and event_source == "local_to_remote":
        return None
    if direction == "remote_to_local" and event_source == "remote_to_local":
        return None

    target_coll = target_db[coll_name]
    # Source tag = direction string (matches anti-loop check)
    source_tag = direction  # "local_to_remote" or "remote_to_local"
    ...
    try:
        if operation_type == "insert":
            ...
            update_fields[SYNC_GEN_FIELD] = 1  # Will be $inc'd
...
def _watch_change_stream(
    ...
    while not stop_event.is_set():
        try:
            ...
            # Build pipeline: watch all collections except skipped
            pipeline = [
            ...
            log.info(f"[{direction}] Change stream active for {db_name}")
            ...
            for change in stream:
                ...
                try:
                    ...
                    # Save resume token after each processed event
                    token = stream.resume_token
                    if token:
                        _save_resume_token(local_db_for_state, db_name, direction, token)
