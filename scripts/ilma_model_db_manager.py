...
class ModelDatabaseManager:
        ...
        for pname, cfg in PROVIDER_CONFIGS.items():
            ...
            if not self.dry_run:
                for mid, meta in parsed:
                    # Self-heal (2026-07-25): a falsy model_id would create a broken
                    # SOT record that fails the integrity gate. Fall back to the dict
                    # key (live id) so the record is always valid; skip only if both
                    # are empty.
                    if not mid:
                        mid = meta.get("id") or meta.get("model_id")
                    if not mid:
                        continue
                    if mid in added:
                        ...
                        # Self-heal (2026-07-25): ensure required identity keys are
                        # always present. Stale records from earlier schema shapes can
                        # lack model_id/provider, which fails the SOT integrity gate.
                        if not cur.get("model_id"):
                            cur["model_id"] = mid
    ...
    def run_passive_benchmark(self) -> Dict[str, Any]:
        """Parse model_usage.jsonl → update benchmark_database.json."""
        ...
        # Load benchmark database for fuzzy model ID matching
        bench_db: Dict = {"benchmarks": {}}
        if BENCH_DB.exists():
            try:
                with open(BENCH_DB) as f:
                    bench_db = json.load(f)
            except Exception:
                pass
        bench_models = set(bench_db.get("benchmarks", {}).keys())

        # Build reverse lookup: usage_log model_id -> matching benchmark model_ids
        def _find_bench_match(usage_mid: str) -> list[str]:
            """Find all benchmark model IDs that match the usage log model ID."""
            matches = []
            # Exact match
            if usage_mid in bench_models:
                matches.append(usage_mid)
            # Prefix match (e.g., "nvidia/meta/llama-3.1-70b-instruct" matches usage "meta/llama-3.1-70b-instruct")
            for bm in bench_models:
                if bm.endswith("/" + usage_mid) or bm == usage_mid:
                    matches.append(bm)
            return matches

        # Aggregate - handle both old format (tokens_used, error_count, success) 
        # and new format (total, success, failed, reliability, avg_latency)
        agg: Dict[str, Dict[str, Any]] = {}
        for e in entries:
            mid = e.get("model_id", "unknown")
            if mid not in agg:
                agg[mid] = {"total_requests": 0, "total_tokens": 0,
                            "total_errors": 0, "total_success": 0, "events": []}
            ...
            if success == 0 and failed == 0:
                success = 1 if e.get("success", True) else 0
                failed = 0 if e.get("success", True) else 1
            
            agg[mid]["total_requests"] += total
            agg[mid]["total_success"]   += success
            agg[mid]["total_errors"]    += failed
        ...
        def _score(a: Dict) -> float:
            ...
            if isinstance(last_ts, str):
                try:
                    ...
                    last_ts = time.time()
            # Recency window: 30 days (2592000 seconds) to accommodate historical data
            rec = max(0.0, 1.0 - (time.time() - last_ts) / 2592000)
        ...
        for mid, a in agg.items():
            ...
            # Find matching benchmark model IDs (fuzzy match)
            bench_matches = _find_bench_match(mid)
            ...
            if not bench_matches:
                # No match in benchmark DB - add new entry with usage log model ID
                new_entry = {
                    "model_id":         mid,
                    "avg_score":        score,
                    "total_requests":   a["total_requests"],
                    "total_tokens":      a["total_tokens"],
                    "total_success":     a["total_success"],
                ...
                # Update all matching benchmark entries
                for bm in bench_matches:
                    existing = bench["benchmarks"].get(bm, {})
                    new_entry = {
                        "model_id":         bm,
                        "avg_score":        score,
                        "total_requests":   a["total_requests"],
    ...
    def sync_artificialanalysis(self) -> Dict[str, Any]:
        ...
        # Cache may be a raw list of records OR a dict with a "records" key.
