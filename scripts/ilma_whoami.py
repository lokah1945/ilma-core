...
Generated: 2026-05-06T23:06:54.906429
=========================================
...
USAGE:
    python3 ilma_whoami.py [OPTIONS]
...
EXAMPLES:
    python3 ilma_whoami.py --verbose
    python3 ilma_whoami.py --dry-run
...
class WhoamiHandler(BaseHandler):
    ...
    def execute(self, *args, **kwargs) -> OperationResult:
        try:
            self.logger.info("Executing Whoami operation")
            ...
            return OperationResult(
                success=True,
                message="Whoami operation completed successfully",
                data=result
            )
        except Exception as e:
    ...
    def _execute_operation(self, *args, **kwargs) -> Any:
        return {"status": "completed", "operation": "Whoami"}

def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog='ilma_whoami.py',
        description='whoami operations',
...
def main() -> int:
    try:
        ...
        if args.verbose:
            logging.getLogger().setLevel(logging.DEBUG)
        logger.info(f"Starting Whoami - SSS Tier Military Grade")
        handler = WhoamiHandler(verbose=args.verbose, dry_run=args.dry_run)
