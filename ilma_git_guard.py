...
SECRET_PATTERNS = [
    ...
    # Refined (2026-07-25): only flag HIGH-ENTROPY assigned values. Placeholder
    # examples in docs such as API_KEY='wrapper-local-key' are non-secrets (low
    # entropy, obviously a local/test value) and must NOT block legitimate commits.
    # Real secrets are >=12 chars matching [A-Za-z0-9_-]{12,} with mixed classes.
    (re.compile(r"(?i)(password|passwd|secret|api[_-]?key|auth[_-]?token)\s*[:=]\s*[\"'](?=.*[A-Za-z])(?=.*[0-9])[A-Za-z0-9_\-]{12,}[\"']"),
