...
Unified health check system for ILMA — consolidates:
  ...
  2. Capability registry health
  3. MASTER DB integrity
  4. Pipeline wiring verification
...
class ILMAHealthCheck:
    ...
    def check_pipeline_wiring(self) -> HealthCheckResult:
        ...
        critical_files = [
            ...
            ("scripts/ilma_passive_benchmark.py", "Passive Benchmark"),
        ]
        ...
        if missing:
            ...
            status = "ERROR"
            details = f"Pipeline wiring: {len(found)}/{len(critical_files)} files present, missing: {missing}"
