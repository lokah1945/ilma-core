...
def _invoke_via_subagent(model_id: str, message: str, max_tokens: int = 2048, timeout: int = 90) -> Tuple[bool, str, float]:
    ...
    try:
        ...
        if "/" in model_id:
            ...
            # Phase 72-fix: route nvidia/* through wrapper-nvidia (local, force-free) first.
            # If model_id starts with "nvidia/", remap to wrapper-nvidia provider.
            if provider == "nvidia":
                provider = "wrapper-nvidia"
            if provider in ("wrapper-nvidia", "openrouter", "blackbox", "minimax", "ollama"):
                try:
                    from ilma_provider_kernel import ProviderKernel
                    kernel = ProviderKernel()
                    # Strip provider prefix — use ORIGINAL provider (before remap) for startswith check.
                    # e.g. "nvidia/meta/llama-3.3-70b-instruct" → "meta/llama-3.3-70b-instruct"
                    orig_prefix = model_id.split("/")[0] + "/"
                    api_model = model_id[len(orig_prefix):] if model_id.startswith(orig_prefix) else model_id
