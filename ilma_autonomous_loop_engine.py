...
2. ANALYSIS — Analyze module health, wiring, import chains
    ...
    def _state_path(self):
        try:
            ...
            logger.warning(f"[AutonomousLoop] swallowed: {_e}")
            return None
    ...
    def _load_persistent_state(self):
        ...
        try:
            ...
            if sp and sp.exists():
                ...
                _hist = d.get("history", [])
                # Harden: history must be a list of dicts; drop malformed entries that
                # would later break .get() calls (e.g. str items from a past bad write).
                if isinstance(_hist, list):
                    self.history = [h for h in _hist[-20:] if isinstance(h, dict)]
                else:
                    self.history = []
        except Exception as _e:
            logger.warning(f"[AutonomousLoop] swallowed: {_e}")
    ...
    def _discovery(self, task: Optional[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            pending = sum(
                s.get("pending", 0) for s in stats.values()
    ...
    def _analyze_capabilities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Harden (2026-07-25): registry entries that are str/non-dict must not
            # break c.get(); skip them instead of swallowing AttributeError.
            missing = sum(
                1 for c in caps
                if isinstance(c, dict) and not c.get("evidence_id")
            )
    ...
    def _analyze_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            unavailable = sum(
                ...
                if isinstance(v, dict) and v.get("unavailable", False)
            )
    ...
    def _auto_resolve_learnings(self) -> int:
        ...
        try:
            ...
            for entry in pending:
                # Harden (2026-07-25): get_pending() returns dicts, but guard against
                # any malformed str/non-dict item that would break entry["id"]/.get().
