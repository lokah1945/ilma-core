...
def check_workflow_pipeline_e2e() -> Dict[str, Any]:
    ...
    expected_connections = {
        ...
        "LAYER_9_SELF_IMPROVE": ["ilma_self_improve_integrator"],
    }
...
def run_full_optimization() -> Dict[str, Any]:
    ...
    if results["health"].get("git_sync", {}).get("uncommitted", 0) > 5:
        ...
        try:
            ...
            logger.debug(f"[Optimizer] feature flag check failed, default gate=True: {_e}")
            _gate_on = True
        ...
        try:
            ...
            logger.debug(f"[Optimizer] git sync subprocess runtime error: {_re}")
        except Exception as e:
