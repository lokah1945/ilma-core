...
class ILMAUnifiedRouter:
    ...
    def __init__(self, allow_paid: bool = False):
        ...
        # ── CIRCUIT BREAKER PERSISTENCE (Fix 2026-07-26 audit) ──────────────
        # Restore in-memory _failure_count / _cooldown_until from persisted health
        # file so a disabled model stays disabled across process restarts.
        # Without this, mark_failure() writes "disabled" to disk but _is_healthy()
        # only reads the in-memory _failure_count (reset to 0 on boot) and never
        # inspects the persisted "disabled" status → circuit breaker is non-durable.
        try:
            _h = self._load_health()
            for _mid, _st in _h.get("models", {}).items():
                _fc = _st.get("failure_count", 0) or _st.get("consecutive_failures", 0) or 0
                if _fc:
                    self._failure_count[_mid] = _fc
                if _st.get("status") == "disabled":
                    _cd = _st.get("cooldown_until")
                    if _cd:
                        try:
                            import datetime as _dt
                            _ts = _dt.datetime.fromisoformat(_cd).timestamp()
                            if _ts > time.time():
                                self._cooldown_until[_mid] = _ts
                        except Exception:
                            self._cooldown_until[_mid] = time.time() + self.CIRCUIT_COOLDOWN
                    else:
                        self._cooldown_until[_mid] = time.time() + self.CIRCUIT_COOLDOWN
        except Exception as _e:
            logger.warning(f"[Router] Failed to restore circuit-breaker state: {_e}")
    ...
    def _load_master_from_mongodb(self) -> Dict:
        ...
        for model in model_docs:
            ...
            record = {
                ...
                # FIX 2026-07-09 (audit F1): single authoritative is_free source.
                # Priority: models collection (model_meta) -> intel doc -> provider free_bypass.
                # REMOVED duplicate "is_free" key (was last-wins overriding the fallback chain).
                "is_free":            model_meta.get("is_free", intel.get("is_free", llm_meta.get("free_bypass", False))),  # Phase R: per-model is_free from models collection (authoritative), fallback to intel/provider bypass
    ...
    def _is_healthy(self, model_id: str) -> bool:
        ...
        if model_state.get("unavailable", False):
            return False
        # Persisted circuit-breaker state must be honored (Fix 2026-07-26 audit):
        # mark_failure() sets status="disabled" on disk, but the in-memory
        # _failure_count is reset on restart — without this check a disabled model
        # would be treated as healthy after a process restart.
        if model_state.get("status") == "disabled":
            return False
        return True
    ...
    def mark_failure(self, model_id: str, error_type: str = "unknown_error",
        ...
        if count >= CircuitBreakerPolicy.DISABLE_THRESHOLD:
            ...
            status = "soft_degraded"  # 1-2 failures = soft degraded (recoverable)
