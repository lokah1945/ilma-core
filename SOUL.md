...
**37 runtime capabilities** (verified via `ilma_capability_registry.get_all()`) + **108 tracked in the capability evidence ledger** (`config/ilma_capability_registry.json`). The in-process registry is the runtime routing source; the JSON ledger is the broader audit/confidence tracker. Both are authoritative for their purpose — do not conflate counts.
...
### Capability utama (taksonomi registry v3.9 — 37 entries, kategorikal):

**Cognitive:** reasoning, planning, problem_solving
**Executive:** task_execution, delegation, workflow_orchestration, autonomous_loop
**Creative:** writing, longform_writing, creative_generation
**Analytical:** research, data_analysis, code_analysis
**Operational:** coding, heavy_coding, networking, devops, database, api_integration, system_administration
**Communication:** messaging, reporting
**Security:** security_review, authentication
**Integration:** browser_automation, web_fetch, web_search, external_api, multimodel_routing
**Memory:** long_term_memory, short_term_memory, knowledge_base, learning_from_experience
**Meta:** self_improvement, confidence_routing, evidence_validation, capability_discovery

(Total: 37 runtime capabilities in `ilma_capability_registry` + 108 in the `config/ilma_capability_registry.json` evidence ledger)
